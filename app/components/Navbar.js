...
export default function Navbar() {
  ...
  const NavLink = ({ href, label, Icon }) => {
    ...
    return (
      <Link
        href={href}
        className={`group relative inline-flex items-center gap-1 px-1.5 lg:px-2 xl:px-3 py-1.5 lg:py-2 text-[10px] lg:text-[11px] xl:text-sm font-semibold transition ${
          active ? "text-slate-900" : "text-slate-600 hover:text-slate-900"
        }`}
      >
        <Icon
          className={`h-3 w-3 lg:h-3.5 lg:w-3.5 xl:h-4 xl:w-4 transition ${
            active ? "text-slate-900" : "text-slate-400 group-hover:text-slate-600"
          }`}
        />
        <span>{label}</span>
        <span
          className={`absolute inset-x-1.5 lg:inset-x-2 bottom-1 h-[1.5px] lg:h-[2px] rounded-full transition ${
  ...
  return (
    ...
    >
      <div className="mx-auto flex w-full max-w-none items-center justify-between gap-1.5 lg:gap-2 xl:gap-4 px-3 py-2.5 sm:px-4 sm:py-3 lg:px-5 lg:py-3 xl:px-10">
        <Link href="/" className="flex items-center gap-1.5 lg:gap-2 xl:gap-3 flex-shrink-0">
          <div className="flex h-9 lg:h-10 xl:h-12 w-auto items-center">
            <img src="/logo1.png" alt="VAWE logo" className="h-9 lg:h-10 xl:h-12 w-auto object-contain" />
          </div>
          <div className="leading-tight text-slate-900">
            <p className="text-sm lg:text-[14px] xl:text-lg font-bold tracking-wide">VAWE</p>
            <p className="text-[9px] lg:text-[10px] xl:text-xs font-semibold uppercase tracking-[0.2em] lg:tracking-[0.25em] xl:tracking-[0.3em] text-slate-600">Global Tech</p>
          </div>
        </Link>

        <nav className="hidden items-center gap-1 lg:gap-1.5 xl:gap-4 px-1 lg:px-2 xl:px-3 py-1.5 lg:py-2 lg:flex">
        ...
        <div className="flex items-center gap-1.5 lg:gap-2">
          <a
            ...
            className="hidden items-center gap-1 lg:gap-1.5 xl:gap-2 rounded-full bg-slate-900 px-2.5 lg:px-3 xl:px-4 py-1.5 lg:py-2 text-[10px] lg:text-[11px] xl:text-sm font-semibold text-white shadow-lg shadow-slate-900/20 transition hover:-translate-y-0.5 hover:shadow-xl lg:flex"
          >
            <Phone className="h-3 w-3 lg:h-3.5 lg:w-3.5 xl:h-4 xl:w-4" />
            <span>Call Us</span>
          </a>
          <button
            ...
            aria-expanded={open}
            onClick={() => setOpen((v) => !v)}
            className="inline-flex items-center gap-2 rounded-full border border-slate-300 bg-white/70 px-3 py-2 text-sm font-semibold text-slate-700 shadow lg:hidden"
          >
