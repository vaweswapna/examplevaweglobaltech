    ...
    <div className="pt-28">
      <section className="w-full px-6">
        <div className="container mx-auto px-2 sm:px-0">
          ...
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="text-2xl pt-2 md:text-5xl font-bold font-(--font-orbitron)" style={{ background: 'var(--vawe-bg-gradient)', WebkitBackgroundClip: 'text', backgroundClip: 'text', color: 'transparent' }}>
            Packages
          </motion.h1>
          <p className="mt-3 md:mt-4 text-xs md:text-base text-neutral-700 max-w-3xl leading-relaxed">Choose a package that fits your stage. Every option blends our glassy, futuristic look with strong foundations. No prices shown — we tailor scope to your needs.</p>

          <div className="mt-6 md:mt-10 grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
            {packages.map((p, i) => (
              <motion.div
                key={p.name}
              ...
              >
                <div className="glass p-4 md:p-6 h-full flex flex-col">
                  ...
                  <div className="flex items-center gap-2 md:gap-3 mt-2">
                    ...
                    <h3 className="text-sm md:text-lg font-semibold" style={{ color: 'var(--vawe-navy)' }}>{p.name}</h3>
                  </div>
                  <p className="text-neutral-700 text-[11px] md:text-sm mt-2">{p.subtitle}</p>
                  <ul className="mt-3 md:mt-4 space-y-1.5 md:space-y-2 text-[11px] md:text-sm text-neutral-700">
                    {p.points.map((pt) => (
                  ...
                  <div className="mt-4 pt-3 md:mt-6 md:pt-4">
                    <a href="/contact" className="btn-primary btn-ring px-3 py-1.5 md:px-4 md:py-2 rounded-full text-[11px] md:text-sm font-semibold hover:-translate-y-px transition" style={{ backgroundColor: p.color, color: '#fff' }}>Discuss Package</a>
                  </div>
                </div>
              </motion.div>
            ))}
